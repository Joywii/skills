# build-independent-review-loops 压力测试结果
...
|---|---|---|---|---|
| `should-trigger-01` | 应调用 | `build-independent-review-loops` | 争议：action | 独立裁决通过 |
| `should-trigger-02` | 应调用 | `build-independent-review-loops` | 通过 | 通过 |
| `should-trigger-03` | 应调用 | `build-independent-review-loops` | 通过 | 通过 |
...
| `edge-02` | 边界 | `build-independent-review-loops` | 通过 | 通过 |
...
- 初始问题：路由正确，但首动作未定义Reviewer可见的独立证据与隔离输入。
- 独立裁决：`grader_overstrict`。定义候选、硬否决项和发布权限正是 Reviewer Skill 的 E1，独立证据与输入隔离是 E2-E3。
- 处理：接受发布契约为首动作，后续再断言独立证据、最小可见字段和结构化审核输出。
